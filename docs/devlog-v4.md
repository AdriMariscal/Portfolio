...
- T-030 [DX]: Recolocar referencia a Salesforce: de stat de hero a About/Services.
  - Patch v4.30.1 — Hotfix: Decap CMS /admin devolvía 404 en config.yml; añadido `<link rel="cms-config-url">` explícito apuntando a `/admin/config.yml`. CSP del admin ampliada (fonts.googleapis.com, unpkg.com, ingesteer.services-prod.nsvcs.net) para evitar bloqueos de recursos. Patch v4.30.2 — Añadido `'unsafe-eval'` a script-src del admin (Decap CMS/AJV compila validadores de schema en runtime). Patch v4.30.3 — Añadido `'unsafe-inline'` a script-src global (/*) para permitir scripts inline de Astro en páginas de blog; posts individuales no cargaban por CSP. Patch v4.30.4 — Hash explícito en script-src para inline de Astro; connect-src ampliado con ingesteer.services-prod.nsvcs.net (RUM Netlify). Patch v4.30.5 — Excluir HTML del precache del SW (globIgnores); el SW servía respuestas cacheadas con CSP obsoleta; ahora las páginas se obtienen por NetworkFirst con headers frescos.
